...
The core state model now supports replacing selected row ids through `selection.replace` and replacing column order through `columns.order.replace`; adapters still need to implement real pointer and keyboard selection behavior.
...
Core state model artik selected row id'lerini `selection.replace` ile ve column order'i `columns.order.replace` ile degistirmeyi destekler; adapter'larin real pointer ve keyboard selection behavior'i henuz implement etmesi gerekir.
