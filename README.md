...
The demo imports package build output from `packages/core/dist/index.js` and `packages/dom/dist/index.js`. The `demo:static` script builds first, then serves the repository root. It exercises `mountStaticGrid`, row refresh, selected-row replacement and column-order replacement through static controls. It does not diff, hydrate, virtualize, sort, filter or handle keyboard interaction.
...
Demo `packages/core/dist/index.js` ve `packages/dom/dist/index.js` build ciktilarini import eder. `demo:static` script'i once build alir, sonra repository root'unu serve eder. `mountStaticGrid`, row refresh, selected-row replacement ve column-order replacement davranisini static control'lerle dener. Diff, hydrate, virtualize, sort, filter veya keyboard interaction yapmaz.
