...
The core state model now supports replacing selected row ids through `selection.replace`, column order through `columns.order.replace`, column visibility through `columns.visibility.replace` and column sizing through `columns.sizing.replace`; adapters still need to implement real pointer and keyboard selection behavior.
...
Core state model artik selected row id'lerini `selection.replace` ile, column order'i `columns.order.replace` ile, column visibility'yi `columns.visibility.replace` ile ve column sizing'i `columns.sizing.replace` ile degistirmeyi destekler; adapter'larin real pointer ve keyboard selection behavior'i henuz implement etmesi gerekir.
