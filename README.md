...
The core state model now supports replacing selected row ids through `selection.replace`, replacing column order through `columns.order.replace` and replacing column visibility through `columns.visibility.replace`; adapters still need to implement real pointer and keyboard selection behavior.
...
The demo imports package build output from `packages/core/dist/index.js` and `packages/dom/dist/index.js`. The `demo:static` script builds first, then serves the repository root. It exercises `mountStaticGrid`, row refresh, selected-row replacement, column-order replacement and column-visibility replacement through static controls. It does not diff, hydrate, virtualize, sort, filter or handle keyboard interaction.
...
Core state model artik selected row id'lerini `selection.replace` ile, column order'i `columns.order.replace` ile ve column visibility'yi `columns.visibility.replace` ile degistirmeyi destekler; adapter'larin real pointer ve keyboard selection behavior'i henuz implement etmesi gerekir.
...
Demo `packages/core/dist/index.js` ve `packages/dom/dist/index.js` build ciktilarini import eder. `demo:static` script'i once build alir, sonra repository root'unu serve eder. `mountStaticGrid`, row refresh, selected-row replacement, column-order replacement ve column-visibility replacement davranisini static control'lerle dener. Diff, hydrate, virtualize, sort, filter veya keyboard interaction yapmaz.
